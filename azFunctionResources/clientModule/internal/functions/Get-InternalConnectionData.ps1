...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'NewPassword.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'NewPassword.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'NewPassword.Client.ProtectedToken'
