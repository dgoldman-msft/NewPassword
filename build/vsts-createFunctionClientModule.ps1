...
$parentModule = 'NewPassword'
if (-not $ModuleName) { $ModuleName = 'NewPassword.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\NewPassword'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\NewPassword.psd1"
$prereqHash = @{
